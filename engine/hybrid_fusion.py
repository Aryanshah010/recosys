...
class HybridFusionEngine:
    ...
    def recommend(self, user_id, user_profile, user_history=None, k=10):
        ...
        candidates.update(
            self.movies_catalog.nlargest(500, "vote_count")["movieId"].tolist()
        )
        ...
        for mid in candidates:
            ...
            if isinstance(matrix_idx, pd.Series):
                if len(matrix_idx) > 1:
                    print(
                        f"WARNING: Multiple indices for movie {mid}: {matrix_idx.values}"
                    )
                matrix_idx = int(matrix_idx.iloc[0])
            else:
            ...
            if valid_history:
                cbf_score = float(
                    np.mean(
                        [
                            self.cosine_sim[self.cbf_indices[h]][matrix_idx]
            ...
            scored_movies.append(
                {
                    "movieId": int(mid),
                    "title": movie_row.get(
                        "title_x", movie_row.get("title", "Unknown")
                    ),
