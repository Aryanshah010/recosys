...
# =====================================================================
# 1. CROSS-DATASET ONTOLOGY MAPPINGS (For Preference Scoring)
# =====================================================================
# Maps synthetic profile strings to TMDb cleaned tokens
GENRE_MAPPING = {
...
def normalize_prefs(profile):
    """Safely parses pipe-separated preferences into TMDb-aligned sets."""
...
# =====================================================================
# 2. DATA PREPARATION & SVD RETRAINING
# =====================================================================
...
# FIX: Safe split that avoids groupby MultiIndex KeyError
# Shuffle first, then split by index to guarantee column preservation
...
# Ensure userId is treated consistently as string for Surprise library
...
# Combine Real + Synthetic Train for SVD
...
# =====================================================================
# 3. EVALUATION LOOP (Optimized Candidate Pool Strategy)
# =====================================================================
print("📊 Running Offline Evaluation on Synthetic Cohort...")

# Filter test users with >= 2 positive ratings for valid NDCG calculation
...
for uid in eval_users:
    # FIX: Cast uid to match profile dtype
    ...
    # Ground Truth: Movies rated >= 4.0 in Test Set for this user
    ...
    valid_history = [m for m in train_history if m in cbf_indices.index]
        ...
        # --- CANDIDATE POOL GENERATION (Saves 90% RAM vs predicting all 43k movies) ---
    candidates = set()
    
    # 1. CBF Candidates (Top 300 similar to history)
    if valid_history:
        # FIX: Safely extract exactly one integer index per movieId
        safe_idxs = []
        for m in valid_history:
            idx_val = cbf_indices.get(m)
            if idx_val is not None:
                # Handle case where get() returns a Series due to duplicates
    ...
    # 2. Popularity Candidates (Top 300 global)
    ...
    for mid in candidates:
        ...
        # 3. Preference Score (Confirmation Bias Injection)
        pref_score = 0.0
        ...
        # --- HYBRID FUSION FORMULAS ---
        model_scores['CF'].append((mid, cf_pred))
        model_scores['CBF'].append((mid, cbf_score))
        
        # Baseline 3: Non-Localized Hybrid (Renormalized 0.5+0.3 → 0.625+0.375)
        model_scores['NonLocal_Hybrid'].append((mid, 0.625 * cf_pred + 0.375 * cbf_score))
        
        # Proposed: Localized Hybrid (Thesis Model)
        if is_cold or not valid_history:
            # Cold Start Renormalization per Thesis_CONTEXT.pdf
            final_score = 0.6 * cbf_score + 0.4 * pref_score
        else:
            # Standard Hybrid: 0.5*CF + 0.3*Content + 0.2*Preference
...
# =====================================================================
# 4. AGGREGATE & SAVE THESIS RESULTS
# =====================================================================
if not results:
    print("⚠️ No valid evaluation results generated. Check data alignment.")
else:
